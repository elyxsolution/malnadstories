...
/**
 * Builder Layout presets (Phase 9E) — applies an ACTIVE preset's geometry to the focused
 * spread. Presets are curated geometry only ({ base, overlays }); applying one produces an
 * ordinary Block via the builder's existing patchBlock, so saveLayout / preview / PDF are
 * unchanged. Placed photos are preserved where possible and never lost.
 *
 * `LayoutPresetGrid` is the reusable body, rendered inline inside the builder's Layouts
 * sidebar tab (Pixory shell). `LayoutPanel` is the legacy slide-over wrapper around it.
 */
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {g.items.map((t) => (
              <button
                ...
                disabled={!hasTarget}
                title={t.name}
                className="group rounded-xl border bg-card p-2 text-left transition-colors hover:border-studio-bright focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-studio-bright disabled:pointer-events-none disabled:opacity-40"
              >
      ...
      <aside className="animate-rise fixed inset-y-0 right-0 z-[56] flex w-[380px] max-w-[92vw] flex-col bg-background shadow-elevated">
        <div className="flex items-start justify-between border-b px-6 py-5">
          <div className="flex items-center gap-2.5">
            <span className="grid h-9 w-9 place-items-center rounded-full bg-secondary text-foreground ring-1 ring-border">
              <LayoutTemplate className="h-[18px] w-[18px]" />
            </span>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">Layouts</p>
              <h2 className="text-[1.35rem] font-semibold leading-tight tracking-tight text-foreground">Pick a layout.</h2>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <LayoutPresetGrid templates={templates} onApply={onApply} hasTarget={hasTarget} />
        </div>

        <p className="flex items-center gap-2 border-t px-6 py-3.5 text-[13px] text-muted-foreground">
          <Sprig className="h-3.5 w-3.5 text-muted-foreground" /> Curated layouts — the final say is always yours.
        </p>
      </aside>
    </>
  );
  ...
  return (
    <div className="relative aspect-[3/2] w-full overflow-hidden rounded-md border bg-muted/40">
      ...
      {geometry.overlays.map((o, i) => (
        <div
          key={i}
          className="absolute rounded-sm border border-studio-bright/70 bg-studio/10"
