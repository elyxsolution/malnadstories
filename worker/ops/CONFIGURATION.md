...
| `APP_URL` | `https://<your-app-domain>` | The origin Chromium loads the print route from. **A wrong value here fails every PDF** and nothing else — validated for shape at boot, not reachability. |
...
| `WV2_PDF_CONCURRENCY` | `1` | A render owns a Chromium page — the memory-dominant operation. This lane is marked *heavy*: while it runs, other lanes automatically halve. **Do not raise without at least 2GB per instance.** |
